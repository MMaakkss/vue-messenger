<template>
	<h1 class="text-center text-3xl">Create an account</h1>
	<div class="form w-1/3 mt-20 mx-auto">
		<input class="block" type="text" placeholder="Name" v-model="name" />
		<input class="block mt-3" type="text" placeholder="Email" v-model="email" />
		<div class="error-message" v-show="invalidEmail">Invalid e-mail</div>
		<div class="pass relative mt-3">
			<div class="absolute cursor-pointer top-2/4 -translate-y-2/4 right-3" @click="showPassword = !showPassword">
			...
			<input class="block" :type="showInputPassword" placeholder="Password" v-model="password" />
		</div>
		<div class="error-message" v-show="invalidPassword">Min length 6</div>
		<div class="mb-2 mt-3 btn" :class="{ disabled: isFormValid }" @click="createAccount">Create an account</div>
